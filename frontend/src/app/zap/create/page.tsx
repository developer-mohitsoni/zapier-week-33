...
function useAvailableActionsAndTriggers(){
  ...
  useEffect(()=>{
    axios.get(`${process.env.NEXT_PUBLIC_API_BASE}/api/v1/trigger/available`).then(x => setAvailableTriggers(x.data.availableTriggers))

    axios.get(`${process.env.NEXT_PUBLIC_API_BASE}/api/v1/action/available`).then(x => setAvailableActions(x.data.availableActions))
  }, [])
...
export default function(){
  ...
  return(
    <div>
      <AppBar/>
      <div className="flex justify-end bg-[#d8d7d5] p-4">
        <PrimaryButton onClick={async ()=>{
          const response = await axios.post(`${process.env.NEXT_PUBLIC_API_BASE}/api/v1/zap`, {
